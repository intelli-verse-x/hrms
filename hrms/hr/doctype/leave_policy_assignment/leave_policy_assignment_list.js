frappe.listview_settings["Leave Policy Assignment"] = {
			...
			me.dialog = new frappe.ui.form.MultiSelectDialog({
				...
				data_fields: [
					...
					},
					{
						...
						onchange: () => {
							if (
								cur_dialog.fields_dict.assignment_based_on.value === "Leave Period"
							) {
								...
								cur_dialog.fields_dict.assignment_based_on.value === "Joining Date"
							) {
	...
	set_effective_date: function () {
		if (
			cur_dialog.fields_dict.assignment_based_on.value === "Leave Period" &&
			cur_dialog.fields_dict.leave_period.value
		) {
